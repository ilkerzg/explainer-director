...
8. **Audio** — instrumental music bed at 0.11 gain with fades + entrance SFX (0.4 gain) at scene-start timestamps; `amix normalize=0` + limiter. See [references/audio.md](references/audio.md).
9. **QA (mandatory)** — contact sheet of plates; cutout facing check; composite mid-frames (subjects face their movement direction — "walking backwards" is a hard fail); text frames at sync+1s (in empty areas, fully crisp); scene-gap table; end-frame artifact scan; `volumedetect` (mix mean ≈ narration mean, max ≈ 0 dB).

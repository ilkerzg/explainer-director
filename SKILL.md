...
5. **AUDIT (mandatory, scripted — not a spot-check)** — run the audit template from [references/scripts.md](references/scripts.md) over EVERY plate before proceeding: vision-check each plate against its persisted plate prompt for {anachronism, text_leak, blank_faces, unwanted_people}; flagged plates get their prompt REWRITTEN with period-accurate wording (LLM) and are regenerated retry-until-clean. A business suit in ancient Rome shipping to the final cut means this step was skipped — that is a process failure, not a model failure. Do not compose until the audit reports zero unresolved flags.
6. **Clips** — ONLY for scenes that truly need generated motion (fire, complex crowd motion): Veo 3.1 Lite primary, grok-imagine fallback. 1-3 clips per video maximum.
7. **Compose** — per-scene ffmpeg: gentle Ken Burns for statics; animated overlay expressions for composites (slide + bob + clamp); word-synced text cards with a **fast pop-in** (scale-settle 1.18x -> 1.0 over ~0.12s, instant out — never fade, never slow wipes); direct cuts; concat; mux narration with `-shortest`. Recipes in [references/motion-composites.md](references/motion-composites.md).
8. **Object beats (mandatory pass)** — word-synced prop pop-ins over the concatenated video: noun literalization IN MODERATION (3-5 strongest moments per minute, never every noun), pop-settle enable-window animation, X-outs, pop SFX beats. Grammar + measured densities in [references/directing.md](references/directing.md). 1-2 inserts and zero pop-ins is under-directed; a beat on every noun is over-directed — both are failures.
9. **Audio** — instrumental music bed at 0.11 gain with fades + entrance SFX (0.4 gain) at scene-start timestamps; `amix normalize=0` + limiter. See [references/audio.md](references/audio.md).
10. **QA (mandatory)** — contact sheet of plates; cutout facing check; composite mid-frames (subjects face their movement direction — "walking backwards" is a hard fail); text frames at sync+1s (in empty areas, fully crisp); scene-gap table; end-frame artifact scan; `volumedetect` (mix mean ≈ narration mean, max ≈ 0 dB).
...
- **Text**: few moments per video, ALWAYS in empty areas reserved in the plate prompt, NEVER over characters; if the text matches narration words it appears word-synced; Baskerville SemiBold plain caps, large, centered in the empty region; FAST pop-in (scale-settle ~0.12s), instant out — never fade, never slow reveals.
